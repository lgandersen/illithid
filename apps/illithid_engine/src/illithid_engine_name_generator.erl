-module(illithid_engine_name_generator).
%% Adjectives and names taken from:
%% https:%%raw.githubusercontent.com/docker/docker-ce/master/components/engine/pkg/namesgenerator/names-generator.go
...
new() ->
    ...
    Names = ?NAMES,
    RandomAdjective = lists:nth(
                        rand:uniform(length(Adjectives)), Adjectives),
    RandomName = lists:nth(
                   rand:uniform(length(Names)), Names),
    io_lib:format("~s_~s", [RandomAdjective, RandomName]).
